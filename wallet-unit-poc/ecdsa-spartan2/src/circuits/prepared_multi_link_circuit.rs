...
use crate::{
    paths::PathConfig,
    utils::{
        calculate_prepared_multi_link_witness_indices, hashmap_to_json_string,
        parse_prepared_multi_link_inputs, parse_witness,
    },
    Scalar, E,
};
...
#[cfg(all(feature = "native-witness", has_circuit_link_2vc))]
witnesscalc_adapter::witness!(link_2vc);

#[cfg(all(feature = "native-witness", has_circuit_link_3vc))]
witnesscalc_adapter::witness!(link_3vc);

#[cfg(all(feature = "native-witness", has_circuit_link_4vc))]
witnesscalc_adapter::witness!(link_4vc);

#[cfg(all(feature = "native-witness", has_circuit_link_2vc))]
fn call_link_2vc_witness(inputs_json: &str) -> Result<Vec<u8>, SynthesisError> {
    link_2vc_witness(inputs_json).map_err(|_| SynthesisError::Unsatisfiable)
}

#[cfg(not(all(feature = "native-witness", has_circuit_link_2vc)))]
fn call_link_2vc_witness(_inputs_json: &str) -> Result<Vec<u8>, SynthesisError> {
    Err(SynthesisError::Unsatisfiable)
}

#[cfg(all(feature = "native-witness", has_circuit_link_3vc))]
fn call_link_3vc_witness(inputs_json: &str) -> Result<Vec<u8>, SynthesisError> {
    link_3vc_witness(inputs_json).map_err(|_| SynthesisError::Unsatisfiable)
}

#[cfg(not(all(feature = "native-witness", has_circuit_link_3vc)))]
fn call_link_3vc_witness(_inputs_json: &str) -> Result<Vec<u8>, SynthesisError> {
    Err(SynthesisError::Unsatisfiable)
}

#[cfg(all(feature = "native-witness", has_circuit_link_4vc))]
fn call_link_4vc_witness(inputs_json: &str) -> Result<Vec<u8>, SynthesisError> {
    link_4vc_witness(inputs_json).map_err(|_| SynthesisError::Unsatisfiable)
}

#[cfg(not(all(feature = "native-witness", has_circuit_link_4vc)))]
fn call_link_4vc_witness(_inputs_json: &str) -> Result<Vec<u8>, SynthesisError> {
    Err(SynthesisError::Unsatisfiable)
}
...
fn call_link_witness(
    credential_count: usize,
    inputs_json: &str,
) -> Result<Vec<u8>, SynthesisError> {
    match credential_count {
        2 => call_link_2vc_witness(inputs_json),
        3 => call_link_3vc_witness(inputs_json),
        4 => call_link_4vc_witness(inputs_json),
        _ => Err(SynthesisError::Unsatisfiable),
    }
}

#[derive(Debug, Clone)]
pub struct PreparedMultiLinkCircuit {
    credential_count: usize,
...
impl PreparedMultiLinkCircuit {
    ...
    fn get_or_generate_witness(&self) -> Result<Vec<Scalar>, SynthesisError> {
        ...
        let inputs = parse_prepared_multi_link_inputs(&json_value)?;
        ...
        let t0 = Instant::now();
        let witness_bytes = call_link_witness(self.credential_count, &inputs_json)?;
        info!(
            "{} witnesscalc time: {} ms",
            self.circuit_name()?,
...
impl SpartanCircuit<E> for PreparedMultiLinkCircuit {
    fn synthesize<CS: ConstraintSystem<Scalar>>(
        ...
        match load_r1cs::<Scalar>(&self.r1cs_path()?) {
            ...
            Err(_) => {
                let layout = calculate_prepared_multi_link_witness_indices(self.n_claims());
    ...
    fn shared<CS: ConstraintSystem<Scalar>>(
        ...
        let device_key_x = witness
            .as_ref()
            .map(|w| w[layout.linked_device_key_x_index])
            .unwrap_or(Scalar::ZERO);
        let device_key_y = witness
            .as_ref()
            .map(|w| w[layout.linked_device_key_y_index])
            .unwrap_or(Scalar::ZERO);
        ...
        let mut shared_values = Vec::with_capacity(2 + layout.linked_claim_values_len);
        shared_values.push(kb_x);
        shared_values.push(kb_y);

        for idx in 0..layout.linked_claim_values_len {
            let claim_scalar = witness
                .as_ref()
                .map(|w| w[layout.linked_claim_values_start + idx])
                .unwrap_or(Scalar::ZERO);
            let claim_alloc =

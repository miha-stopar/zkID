...
use crate::{
    paths::PathConfig,
    utils::{
        calculate_prepare_2vc_output_indices, hashmap_to_json_string_prepare_2vc, parse_witness,
    },
    Scalar, E,
};
#[cfg(feature = "native-witness")]
use crate::utils::parse_prepare_2vc_inputs;
#[cfg(feature = "native-witness")]
...
pub(crate) fn call_prepare_2vc_witness(
    ...
    let result = match circuit_name {
        ...
        "prepare_2vc_1k" => prepare_2vc_1k_witness(inputs_json),
        #[cfg(has_circuit_prepare_2vc_2k)]
        "prepare_2vc_2k" => prepare_2vc_2k_witness(inputs_json),
        #[cfg(has_circuit_prepare_2vc_4k)]
        "prepare_2vc_4k" => prepare_2vc_4k_witness(inputs_json),
        #[cfg(has_circuit_prepare_2vc_8k)]
        "prepare_2vc_8k" => prepare_2vc_8k_witness(inputs_json),
        name => {
            eprintln!(
                "Circuit '{}' is not compiled into this binary.\n\
                 Run circom compile for prepare_2vc_* then `cargo build --release`.",
                name
            );
...
/// Track A (Option 2): dual SD-JWT Prepare — one proof, shared slice with Show(4,…).
#[derive(Debug, Clone)]
...
impl Prepare2VcCircuit {
    ...
    fn resolve_input_json(&self) -> PathBuf {
        self.input_path
            ...
            .unwrap_or_else(|| self.path_config.input_json("prepare_2vc"))
    }
    ...
    fn get_or_generate_witness(&self) -> Result<Vec<Scalar>, SynthesisError> {
        ...
        let witness = generate_prepare_2vc_witness_local(
            &self.path_config,
...
/// Native witness generation for `prepare_2vc_*` (kept here to avoid a crate cycle with `prover`).
#[cfg(feature = "native-witness")]
fn generate_prepare_2vc_witness_local(
    config: &PathConfig,
    input_json_path: Option<&std::path::Path>,
) -> Result<Vec<Scalar>, SynthesisError> {
    let json_path = input_json_path
        .map(|p| config.resolve(p))
        .unwrap_or_else(|| config.input_json("prepare_2vc"));
    ...
    let name = config.circuit_size.prepare_2vc_circuit_name();
    let witness_bytes = call_prepare_2vc_witness(&name, &inputs_json)?;

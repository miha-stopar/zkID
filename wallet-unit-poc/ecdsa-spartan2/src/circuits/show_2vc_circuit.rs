...
/// Track A: `Show(4, 2, 8, 64)` — shared slice matches [`Prepare2VcCircuit`](super::prepare_2vc_circuit::Prepare2VcCircuit).
#[derive(Debug, Clone)]
...
impl Show2VcCircuit {
    ...
    fn resolve_input_json(&self) -> PathBuf {
        self.input_path
            ...
            .unwrap_or_else(|| self.path_config.input_json("show_2vc"))
    }
    ...
    fn get_or_generate_witness(&self) -> Result<Vec<Scalar>, SynthesisError> {
        ...
        info!("Generating witness using witnesscalc (show_2vc)...");
        let t0 = Instant::now();
...
impl SpartanCircuit<E> for Show2VcCircuit {
    ...
    fn shared<CS: ConstraintSystem<Scalar>>(
        ...
        let layout = calculate_show_witness_indices(CircuitSize::TRACK_A_2VC_N_CLAIMS);
